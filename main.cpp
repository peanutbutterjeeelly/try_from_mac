...
private:
	...
	stack<TreeNode*> stk;
public:
	BSTIterator_iterative(TreeNode* root):cur(root){}
...
class find_min_in_rotated_array{
public:
...
public:
	...
	//当前输入查询字符在查询字符串的下标
	vector<string> res;
	//目前所有可能的补全结果，按题目要求排序（热度递减、字典序递增）
	unordered_map<char, map<string, int>> cnt;
	//以各个字母开头的所有字符串的历史查询次数
...
//给定 word = "ABCB", 返回 false
	...
	construct_binaryTree_from_pre_in_order binary_tree_constructor;
	...
	cout << search_range.searchRange(search_range_vec, 8)<<endl;
	...
	Bst_Root->left = Bst_Root_Left;
	Bst_Root->right = Bst_Root_Right;
	Bst_Root_Right->left = Bst_Root_Right_Left;
	Bst_Root_Right->right = Bst_Root_Right_Right;
